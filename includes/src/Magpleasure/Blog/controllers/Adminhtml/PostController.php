...
class Magpleasure_Blog_Adminhtml_PostController extends Magpleasure_Blog_Controller_Adminhtml_Filterable
{
    ...
    public function saveAction()
    {
		...
		//save image
				try{
			if((bool)$requestPost['postimage']['delete']==1) {
						$requestPost['postimage']='';
			} else {
				unset($requestPost['postimage']);

				if (isset($_FILES)){

					if ($_FILES['postimage']['name']) {

						$path = Mage::getBaseDir('media')  . DS . 'blogpost' . DS .'image'.DS;;
						$uploader = new Varien_File_Uploader('postimage');
						$uploader->setAllowedExtensions(array('jpg','jpeg','png','gif'));
						$uploader->setAllowRenameFiles(false);
						$uploader->setFilesDispersion(false);
						//$destFile = $path.$_FILES['postimage']['name'];
						...
						$requestPost['postimage']='blogpost/image/'.$filename;
					}
				}
			}
